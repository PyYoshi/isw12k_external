    ...
    if (!error) {
      ...
      if (error_fn) {
        ...
        if (!jni::CheckException(env)) {
          ...
          } else if (code == 0) {  // SSL_NOTYETVALID == 0
            result = VERIFY_NOTYETVALID;
          } else if (code == 1) {  // SSL_EXPIRED == 1
            result = VERIFY_EXPIRED;
          } else if (code == 4) {  // SSL_DATE_INVALID == 4
            result = VERIFY_DATE_INVALID;
          }
        }
      }
