				...
				uint16_t content_protection)
{
	...
	codec->lock = lock;
	codec->content_protection = content_protection;
					...
					void *user_data)
{
	...
	for (; seps; seps = g_slist_next(seps)) {
		...
		GSList *cl;
		struct avdtp_service_capability *prot;
		uint16_t content_protection = 0;
		...
		prot = avdtp_get_remote_sep_protection(rsep);
		if (prot && (prot->length >= 2)) {
			struct avdtp_content_protection_capability *prot_cap = (void *) prot->data;
			content_protection = (prot_cap->cp_type_msb << 8) | prot_cap->cp_type_lsb;
		}

		a2dp_append_codec(rsp, cap, seid, type, configured, lock, content_protection);
	}
...
static int handle_a2dp_transport(struct unix_client *client,
				struct bt_set_configuration_req *req)
{
	struct avdtp_service_capability *media_transport, *media_codec, *media_scms_t;
	struct sbc_codec_cap sbc_cap;
	struct mpeg_codec_cap mpeg_cap;
	struct avdtp_content_protection_capability *prot_cap;
	...
	if (req->codec.content_protection == BT_CONTENT_PROTECTION_SCMS_T) {
		prot_cap = avdtp_get_scms_t_cap();

		media_scms_t = avdtp_service_cap_new(AVDTP_CONTENT_PROTECTION, prot_cap,
							sizeof(*prot_cap));
