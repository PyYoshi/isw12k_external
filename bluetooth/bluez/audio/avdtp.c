...
{
	GSList *l;
	struct avdtp_remote_sep *sep_found = NULL;
	...
	for (l = session->seps; l != NULL; l = g_slist_next(l)) {
		...
		if (sep->stream == NULL) {
			if (avdtp_remote_sep_has_scms_t_cap(sep))
				return sep;
...
struct avdtp_content_protection_capability *avdtp_get_scms_t_cap(void)
{
	const static struct avdtp_content_protection_capability scms_t_cap = {
		(AVDTP_PROTECTION_TYPE_SCMS_T >> 0) & 0xFF,
		(AVDTP_PROTECTION_TYPE_SCMS_T >> 8) & 0xFF,
	};
	return &scms_t_cap;
}

static gboolean is_scms_t_cap(struct avdtp_service_capability *caps)
{
	if (caps) {
		struct avdtp_content_protection_capability *prot_cap = avdtp_get_scms_t_cap();
		return (memcmp(caps->data, prot_cap, sizeof(*prot_cap)) == 0);
	}
	return FALSE;
}

gboolean avdtp_remote_sep_has_scms_t_cap(struct avdtp_remote_sep *sep)
{
	return is_scms_t_cap(avdtp_get_remote_sep_protection(sep));
}

gboolean avdtp_stream_has_scms_t_cap(struct avdtp_stream *stream)
{
	return is_scms_t_cap(avdtp_get_protection(stream));
}
