...
static void sink_scms_t_state_signal(struct audio_device *dev, gboolean value)
{
	emit_property_changed(dev->conn, dev->path,
				AUDIO_SINK_INTERFACE, "SCMS_T",
				DBUS_TYPE_BOOLEAN, &value);
}
					...
					void *user_data)
{
	...
	case AVDTP_STATE_CONFIGURED:
		sink_scms_t_state_signal(dev, avdtp_stream_has_scms_t_cap(stream));
		break;
				...
				void *user_data)
{
	...
	id = a2dp_select_capabilities(sink->session, AVDTP_SEP_TYPE_SINK, NULL,
						select_complete, sink, sink->protection_required);
