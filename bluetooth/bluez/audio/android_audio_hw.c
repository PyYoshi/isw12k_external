...
#define SCMST_CP_HEADER                "scmst_cp_header"
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    ret = str_parms_get_int(parms, SCMST_CP_HEADER, &scmst);

    if (ret >= 0) {
        if (adev->output && adev->output->data){
            a2dp_set_cp_header(adev->output->data, (uint8_t)scmst);
        }
    }
