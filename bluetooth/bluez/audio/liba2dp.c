					...
					struct bt_get_capabilities_rsp *rsp)
{
	...
	codec_capabilities_t *codec_found = NULL;
	...
	while (bytes_left > 0) {
		if ((codec->type == BT_A2DP_SBC_SINK) &&
			...
			if (codec->content_protection == BT_CONTENT_PROTECTION_SCMS_T)
				break;
		}
